...
output:
  xaringan::moon_reader:
    self_contained: false # SET TO TRUE!!!
...
## All slides available at [ADD LINK](https://www.google.com)
...
.pull-right[
- Member of the [Coastal Ecology and Conservation Lab](https://www.jacoballgeier.com)

- Working on **individual-based simulation models**, but also interested in **landscape ecology**, **point pattern analysis** and spatial data

- Maintainer of several _R_ packages: _landscapemetrics_ .ref[(Hesselbarth et al. 2019)], _shar_ .ref[(Hesselbarth 2021)], _arrR_ .ref[(Esquivel et al. 2022)] and others
]
...
- [_R_](https://www.r-project.org) is **free**, **open-source**, and **multi-platform** (Windows, macOS, Linux)
...
- Includes syntax-highlighting, auto-completion, figure panel, help panel, ...
...
- Once objects are assigned using `<-` operator, they can be **reused**
...
- `list()`
List of different types and/or structure (allows all of above)
]
...
```{r vectorization}
(extinct <- c("amphibians" = 35, "birds" = 159, "mammals"  = 85))
...
log(mean(rescale(penguins$body_mass_g, 
                 to = c(0, 1)), na.rm = TRUE))
```
]
...
- Very powerful resource to understand functions
]
...
Work on _exercise-intro.Rmd_ file ADD LINK! 
...
# create new columns
penguins$body_mass_kg <- penguins$body_mass_g / 10000
...
- Many useful functions (mainly) to deal with _data.frames_
...
- Many **descriptive** functions e.g., `min()`, `max()`, `mean()`, `sd()`, `range()`, `quantile()`, `density()`, etc.
...
```{r t_test}
# extracte body mass for male and female and run t test
male <- penguins[penguins$sex == "male", "body_mass_g", drop = TRUE]
female <- penguins[penguins$sex == "female", "body_mass_g", drop = TRUE]
...
```{r predict_plot, echo = FALSE, fig.align = "center", out.width = "35%"}
...
Work on _exercise-stats.Rmd_ file
...
# create simple scatter plot bodymass vs bill length
...
# add legend
legend("topleft", legend = c("Chinstrap", "Gentoo", 
                             "Adelie"), 
       col = color_palette, pch = 19)
```
]
...
df_sum <- dplyr::group_by(penguins, sex) %>% 
...
- Code is **repeated** until conditions is met

- `for` and `while` loops most common

- Can be useful if vectorization is not possible, but **can be slow** if used incorrectly (never grow a vector, always pre-allocate!)

.pull-left[
```{r loop}
result_good <- numeric(length = 5)
result_bad <- numeric()
...
.pull-left[
- Check if **logical statement** is true and run different parts of code
